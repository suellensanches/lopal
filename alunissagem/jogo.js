...
let lancamento = (Math.round(Math.random()) == 0); //variável booleana pseudoaleatoria
...
function mostrarAngulo(){
    ...
    contexto.fillText(
        `Angulo: ${(moduloLunar.angulo *180 / Math.PI ).toFixed(0)}`,
        400,
        40
    );
} 
...
function encerrarJogo(){
       if(moduloLunar.posicao.y > canvas.height - moduloLunar.altura* 0.5){
        if(moduloLunar.velocidade.y <= 0.5 &&
            ...
            moduloLunar.angulo <= 0.5){
             mostrarResultado("Você pousou com sucesso!", cor = "green")
        }else {
            mostrarResultado("Você não pousou muito bem...", cor = "yellow")
        }
        return true;
    }
    return false;
